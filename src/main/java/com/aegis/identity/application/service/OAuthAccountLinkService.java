...
public class OAuthAccountLinkService {
  ...
  public AuthenticationResult execute(LinkAccountCommand command) {
    ...
    IdentityProvider targetProvider =
        command.provider() != null ? command.provider() : IdentityProvider.GOOGLE;

    Optional<UserIdentity> existing =
        userIdentityRepository.findByProviderAndProviderSubject(
            targetProvider, command.providerSubject());
    ...
    UserIdentity newIdentity =
        new UserIdentity(user, targetProvider, command.providerSubject(), command.email());
  ...
  @Transactional
  public UserIdentity linkAuthenticatedUser(
      UUID userId, IdentityProvider provider, String providerSubject, String providerEmail) {
    User user =
        userRepository
            .findById(userId)
    ...
    Optional<UserIdentity> existing =
        userIdentityRepository.findByProviderAndProviderSubject(targetProvider, providerSubject);
    if (existing.isPresent()) {
      if (existing.get().getUser().getId().equals(user.getId())) {
        throw new IllegalStateException("External identity is already linked to this account");
      } else {
