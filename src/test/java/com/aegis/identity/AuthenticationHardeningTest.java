...
class AuthenticationHardeningTest {
  ...
  @Test
  @DisplayName(
      "Hardening: Using a refresh token as an access token on protected routes must be rejected")
  void testRefreshJwt_usedAsAccessJwt_isRejected() throws Exception {
    ...
    mockMvc
        .perform(get("/api/aegis/v1/auth/me").header("Authorization", "Bearer " + refreshToken))
  ...
  void testPasswordLengthValidation_shortPassword_rejected() throws Exception {
    RegisterRequest shortPwdRequest =
        new RegisterRequest(
            "Short Pwd Org",
            "short-pwd-org-" + UUID.randomUUID(),
            "Default Workspace",
            "default",
  ...
  @Test
  @DisplayName(
      "Hardening: Excessively long password (>128 chars) must be rejected to prevent BCrypt DoS")
  void testPasswordLengthValidation_longPassword_rejected() throws Exception {
    String longPwd = "a".repeat(129); // > 128 characters
    RegisterRequest longPwdRequest =
        new RegisterRequest(
            "Long Pwd Org",
            "long-pwd-org-" + UUID.randomUUID(),
            "Default Workspace",
            "default",
  ...
  @Test
  @DisplayName(
      "Hardening: Invalid login credentials must return 401 Unauthorized without leaking detail")
