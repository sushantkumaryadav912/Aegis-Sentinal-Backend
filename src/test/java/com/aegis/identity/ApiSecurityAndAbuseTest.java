...
class ApiSecurityAndAbuseTest {
    ...
    @Test
    @DisplayName("5.5 HTTP Security: Response headers must enforce frame options, content-type nosniff, referrer, and permissions policy")
    ...
    @Test
    @DisplayName("5.6 CORS: OPTIONS preflight request must return allowed origin, methods, and headers")
    void testCorsPreflightRequest_returnsAllowedOriginsAndMethods() throws Exception {
        mockMvc.perform(options("/api/aegis/v1/auth/login")
    ...
    @Test
    @DisplayName("5.7 Rate Limiting: Exceeding auth request limit from single IP returns 429 Too Many Requests with Retry-After header")
    void testRateLimiting_excessiveAuthRequests_returns429() throws Exception {
        ...
        MvcResult result = mockMvc.perform(post("/api/aegis/v1/auth/login")
                        .header("X-Forwarded-For", testIp)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(req)))
    ...
    @Test
    @DisplayName("5.8 Input & Abuse: Malformed JSON payload returns 400 Bad Request with clean JSON error body")
